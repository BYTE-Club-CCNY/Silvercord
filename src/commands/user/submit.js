  ...
  async execute(interaction) {
    ...
    try {
      ...
      const extractResponse = await fetch(
        `${API_BASE_URL}/leetcode/extract-problem?link=${encodeURIComponent(user_link)}`,
      );
      if (!extractResponse.ok) {
        interaction.followUp(
          `Link ${user_link} is in the wrong format, resubmit the link that contains the submission ID`,
        );
        return;
      }
      ...
      const problemsResponse = await fetch(
        `${API_BASE_URL}/problems?server_id=${server_id}&user_id=${user_id}`,
      );
      if (!problemsResponse.ok) {
        interaction.followUp(
          "Failed to fetch your problems. Please try again.",
        );
        return;
      }
      ...
      const difficultyResponse = await fetch(
        `${API_BASE_URL}/leetcode/difficulty?link=${encodeURIComponent(user_link)}`,
      );
      if (!difficultyResponse.ok) {
        interaction.followUp(
          "Failed to fetch problem difficulty. Please try again.",
        );
        return;
      }
      ...
      const scoreResponse = await fetch(
        `${API_BASE_URL}/scores?server_id=${server_id}&user_id=${user_id}`,
      );
      if (!scoreResponse.ok) {
      ...
      const leaderboardResponse = await fetch(
        `${API_BASE_URL}/scores/leaderboard?server_id=${server_id}`,
      );
      ...
      if (leaderboard && leaderboard.length > 1) {
        for (let i = 1; i < leaderboard.length; i++) {
          if (
            leaderboard[i].user_id == user_id &&
            leaderboard[i].score == prev_score
          ) {
            if (leaderboard[i - 1].user_id != user_id) {
              ...
              leaderboard[i + 1].user_id != user_id &&
              leaderboard[i + 1].score == prev_score
            ) {
              nextUserID = leaderboard[i + 1].user_id;
              nextUserScore = leaderboard[i + 1].score;
            }
